...
public class PaperTest {
    ...
    public void should_create_paper_with_new() {
        ...
        List<BlankQuiz> blankQuizzes = Arrays.asList(
                ...
                new BlankQuiz(fifthQuizId, 10)
        );

        Paper paper = Paper.assemble(blankQuizzes, teacherId);

        assertThat(paper).isNotNull();
        assertThat(paper.getBlankQuizzes().size()).isEqualTo(5);
    }

    @Test(expected = IllegalQuizzesCountException.class)
    public void should_quizzes_quantity_less_than_20() {
        ...
        Paper.assemble(blankQuizzes, teacherId);
    }
}
